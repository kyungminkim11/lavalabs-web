(()=>{
  ...
  document.querySelectorAll('.dropdown-menu').forEach(menu=>{
    ...
    item.href=href;
    item.innerHTML='<span class="dropdown-icon">EDIT</span><span><strong>360 이미지 편집기</strong><small>실시간 보정·바닥 로고·안전 저장</small></span>';
  ...
  document.querySelectorAll('.footer-links').forEach(group=>{
    ...
    const item=document.createElement('a');item.href=href;item.textContent='무료 360 이미지 편집기';
    spaceLink.insertAdjacentElement('afterend',item);
  });
  ...
  const addServiceCard=grid=>{
    ...
    card.innerHTML='<span class="service-no">FREE · 360 TOOL</span><h3>촬영한 360 이미지 직접 확인·보정</h3><p>브라우저에서 360 화면을 둘러보고 색감, 바닥 로고와 시작 방향을 정리해 저장합니다.</p><ul><li>실시간 보정·원본 비교</li><li>로고·단색·텍스트 바닥 패치</li><li>기기별 안전 출력·GPano 저장</li></ul><span class="service-link">무료 편집기 사용하기 →</span>';
  ...
  if(current==='services.html')addServiceCard(document.querySelector('main .service-grid'));
})();