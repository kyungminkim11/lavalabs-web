(()=>{
  ...
  if(floorplan){
    floorplan.innerHTML=`<svg viewBox="0 0 900 520" role="img" aria-label="실제 배치 기반 파티룸 안내 도면. 출입문은 왼쪽 아래에 있습니다.">
      ...
      <ellipse cx="120" cy="174" rx="74" ry="102" fill="#ede7ff" stroke="#d6cbea" stroke-width="2"/>
      <text class="map-label" x="120" y="170" text-anchor="middle">홀덤</text>
      <text class="map-small" x="120" y="194" text-anchor="middle">포커 테이블</text>
      <rect x="210" y="96" width="96" height="150" rx="14" fill="#e8f4ff" stroke="#cbd8e4" stroke-width="2"/>
      <text class="map-label" x="258" y="175" text-anchor="middle" transform="rotate(-90 258 175)">소파</text>
      <rect x="346" y="46" width="156" height="60" rx="12" fill="#fff2cf" stroke="#e3d5ab" stroke-width="2"/>
      <text class="map-small" x="424" y="82" text-anchor="middle">전자레인지</text>
      <rect x="390" y="138" width="76" height="96" rx="10" fill="#202734"/>
      <text x="428" y="194" text-anchor="middle" font-size="22" font-weight="950" fill="#fff">TV</text>
      <text class="map-small" x="428" y="258" text-anchor="middle">노래방 · PS5</text>
      ...
      <text class="map-label" x="682" y="424" text-anchor="middle">PC 게임존</text>
      <path d="M58 485H200" fill="none" stroke="#fff" stroke-width="12"/>
      <path d="M200 485V343" fill="none" stroke="#8d98a5" stroke-width="5"/>
      <path d="M58 485A142 142 0 0 1 200 343" fill="none" stroke="#c7d0d9" stroke-width="3" stroke-dasharray="8 7"/>
      <text class="map-small" x="129" y="470" text-anchor="middle" paint-order="stroke" stroke="#fff" stroke-width="5">출입문</text>
    </svg>
    <button class="marker active" style="left:50%;top:57%" data-scene="overview" aria-label="전체 공간으로 이동" aria-pressed="true">01</button>
    <button class="marker" style="left:29%;top:42%" data-scene="lounge" aria-label="TV 라운지로 이동" aria-pressed="false">02</button>
    <button class="marker" style="left:69%;top:47%" data-scene="tablepc" aria-label="다인석 테이블로 이동" aria-pressed="false">03</button>
    <button class="marker" style="left:76%;top:82%" data-scene="pc" aria-label="PC 게임존으로 이동" aria-pressed="false">04</button>
    <button class="marker" style="left:89%;top:35%" data-scene="kitchen" aria-label="주방으로 이동" aria-pressed="false">05</button>`;
  }

  let viewer=null;
  let viewerInitialized=false;
  let pendingScene="overview";

  function track(name,params={}){
    try{
      if(typeof window.gtag==="function")window.gtag("event",name,params);
      window.dispatchEvent(new CustomEvent("lavalabs:analytics",{detail:{name,params}}));
    }catch(error){console.warn("analytics event skipped",error)}
  }
  ...
  function loadScene(id){
    ...
    track("case_scene_select",{scene:id});
  }
  ...
  if(sceneList){
    Object.entries(sceneInfo).forEach(([id,scene])=>{
      ...
      button.setAttribute("aria-pressed",String(id==="overview"));
  ...
  function showViewerFallback(message="네트워크를 확인한 뒤 새로고침해 주세요."){
    const panorama=document.getElementById("panorama");
    const loading=document.getElementById("viewerLoading");
    loading?.classList.add("hidden");
    if(panorama)panorama.innerHTML=`<div class="viewer-fallback"><b>360° 투어를 불러오지 못했습니다.</b><span>${message}</span><a href="/partyroom/" target="_blank" rel="noopener">실제 사이트에서 사진 보기 ↗</a></div>`;
  }
  ...
  function initViewer(){
    ...
    const panorama=document.getElementById("panorama");
    if(!panorama)return;
    if(!window.pannellum){showViewerFallback();return}
    try{
      const root="/works/partyroom-360-tour/assets/";
      viewer=window.pannellum.viewer("panorama",{
        default:{firstScene:pendingScene,autoLoad:true,sceneFadeDuration:650,showControls:true,compass:false,hfov:102,minHfov:45,maxHfov:120},
        scenes:{
      ...
      viewer.on("load",()=>document.getElementById("viewerLoading")?.classList.add("hidden"));
      viewer.on("error",()=>showViewerFallback("잠시 후 다시 시도하거나 실제 사이트의 일반 사진을 확인해 주세요."));
      syncScene(pendingScene);
      track("case_tour_loaded");
    }catch(error){
      console.warn(error);
      showViewerFallback();
    }
  ...
  if("IntersectionObserver" in window&&tourSection){
    const viewerObserver=new IntersectionObserver(entries=>{
      if(entries.some(entry=>entry.isIntersecting)){initViewer();viewerObserver.disconnect()}
    },{rootMargin:"300px 0px"});
    viewerObserver.observe(tourSection);
  }else{
    initViewer();
  }
  syncScene("overview");
  ...
  if("IntersectionObserver" in window&&trackedSections.length){
    ...
    },{rootMargin:"-32% 0px -58%",threshold:[0,.15,.35]});
  ...
  function enhanceUrl(element,extra={}){
    try{
      const url=new URL(element.getAttribute("href"),location.origin);
      allowedUtm.forEach(key=>{if(currentParams.has(key)&&!url.searchParams.has(key))url.searchParams.set(key,currentParams.get(key))});
      Object.entries(extra).forEach(([key,value])=>{if(value&&!url.searchParams.has(key))url.searchParams.set(key,value)});
      element.href=url.pathname+url.search+url.hash;
    }catch(error){console.warn("link enhancement skipped",error)}
  }
  document.querySelectorAll(".contact-link").forEach((link,index)=>enhanceUrl(link,{utm_content:link.dataset.track||`case_contact_${index+1}`}));
  document.querySelectorAll("[data-live-site]").forEach(link=>enhanceUrl(link,{utm_source:"lavalabs",utm_medium:"case-study",utm_campaign:"partyroom-360"}));

  document.querySelectorAll("[data-track]").forEach(element=>element.addEventListener("click",()=>track(element.dataset.track,{path:location.pathname})));
  ...
  function renderPhoto(index){
    ...
    caption.textContent=`${currentPhoto+1} / ${photoButtons.length} · ${item.dataset.caption||""}`;
  }
  function openLightbox(index){
    ...
    track("case_photo_open",{index:index+1,caption:photoButtons[index].dataset.caption||""});
  }
  ...
  if(lightbox){
    ...
    lightbox.addEventListener("touchend",event=>{
      ...
      if(Math.abs(diff)>55)renderPhoto(diff>0?currentPhoto-1:currentPhoto+1);
    },{passive:true});
  }
  ...
  document.querySelectorAll(".faq-list details").forEach(details=>details.addEventListener("toggle",()=>{
    ...
    track("case_faq_open",{question:details.querySelector("summary")?.textContent||""});
  }));
  ...
  document.querySelectorAll("img").forEach(image=>image.addEventListener("error",()=>{
    ...
    if(picture){picture.classList.add("image-error");image.hidden=true}
